...
function init() {
    ...
    var uniforms = {
        pointTexture: { value: new THREE.TextureLoader().load( 'static/sprites/disk.png' ) },
...
function loadManifest() {
    loader.load('static/data/manifest.json',onHaveManifest);
}
...
function beginLoadParticleCloud(key) {
    ...
    if (key in localDataSets) {
        ...
        var url="static/data/".concat(manifest[settings.category][key]);
        sljloader.load(
            url,
